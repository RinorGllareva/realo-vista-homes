...
const AreaFilter: React.FC<AreaFilterProps> = ({ applyAreaFilter, className, scrolled }) => {
  const [open, setOpen] = useState(false);
  const [minArea, setMinArea] = useState("");
  const [maxArea, setMaxArea] = useState("");
  const ref = useRef<HTMLDivElement | null>(null);
  const active = Boolean(minArea || maxArea);
  ...
  const apply = () => {
    ...
    applyAreaFilter(Number.isNaN(min as number) ? null : min, Number.isNaN(max as number) ? null : max);
    setOpen(false);
  };
  ...
  return (
    <div ref={ref} className={`relative ${className ?? ""}`}>
      ...
      >
        <FiLayout className="text-sm md:text-base" />
        Sipërfaqja
      </button>

      {open && (
        <div className="absolute right-0 z-50 mt-3 w-[min(92vw,360px)] rounded-lg border border-slate-200 bg-white p-3 shadow-2xl">
            ...
            {[
              ...
              <button
                ...
                onClick={() => {
                  ...
                  applyAreaFilter(min as number | null, max as number | null);
                  setOpen(false);
                }}
